...
class TemporalReprojectMaterial extends ShaderMaterial {
  constructor(textureCount = 1) {
    ...
    finalFragmentShader =
      definitions + finalFragmentShader.replaceAll('textureCount', textureCount);
  ...
  setSize(width, height) {
    ...
    for (let i = 0; i < this.textureCount; i++) {
      ...
      const accumulatedTexture =
        (_this$overrideAccumul = this.overrideAccumulatedTextures[i]) !== null &&
        _this$overrideAccumul !== void 0
...
class DenoiserComposePass extends Pass {
  constructor(camera, textures, gBufferTexture, depthTexture, options = {}) {
    ...
    this.fullscreenMaterial = new ShaderMaterial({
      ...
      defines: {
        inputType:
          (_indexOf = ['diffuseSpecular', 'diffuse', 'specular'].indexOf(options.inputType)) !==
  ...
  constructor(scene, camera, texture, options = defaultDenosierOptions) {
    ...
    const composerInputTextures =
      (_this$denoisePass$tex =
        (_this$denoisePass = this.denoisePass) == null ? void 0 : _this$denoisePass.texture) !==
      ...
      Object.defineProperty(this, key, {
        ...
        set(value) {
          ...
          switch (key) {
            ...
            case 'depthPhi':
            case 'normalPhi':
              this.PoissonDenoisePass.fullscreenMaterial.uniforms[key].value = Math.max(
                value,
                0.0001
              );
              break;
...
class TAAPass extends Pass {
  ...
  render(renderer, inputBuffer) {
    ...
    const cameraMoved =
      this.needsUpdate ||
      didCameraMove(this._camera, this.lastCameraPosition, this.lastCameraQuaternion);
    this.needsUpdate = false;
